...
#' @description \R does not store nor export the path of the currently running script.
...
#' @details This functions currently work only if the script was `source`d, processed with `knitr`,
#'   or run with `Rscript` or using the `--file` parameter to the `R` executable.
#'   For code run with `Rscript`, the exact value of the parameter passed to `Rscript` is returned.
